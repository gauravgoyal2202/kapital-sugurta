#!/bin/bash
# =============================================================================
#  run_etl_windows.sh — Kapital Sugurta Core ETL Pipeline (Windows/Git Bash)
...
# ── Configuration ─────────────────────────────────────────────────────────────
PIPELINE_NAME="Core ETL Pipeline (run_etl_windows.sh)"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
if [ -f ".venv/Scripts/activate" ]; then
    STEP2_ERR=$(source .venv/Scripts/activate 2>&1)
else
...
if [[ $? -ne 0 ]]; then
    send_alert "Virtual Environment Activation" \
        ...
        "Expected path" "$(pwd)/.venv/Scripts/activate or .venv/bin/activate"
fi
